...
export default function App() {
  ...
  useEffect(() => {
    (window as any).showPublicBooking = () => setShowBookingForm(true);
  }, []);
  ...
  useEffect(() => {
    if (user && import.meta.env.VITE_FUNCTIONS_URL) {
      fetch(`${import.meta.env.VITE_FUNCTIONS_URL}/manualSyncCalendarSlots`, { method: 'POST' })
        ...
        .then(data => {
          if (data.success) {
            console.log(`Calendar synced: ${data.count} slots blocked.`);
          }
        })
  ...
  const handleLogin = async () => {
    try {
      ...
      toast.error(`Nepodarilo sa spustiť prihlásenie: ${error.message}`);
    }
  };
  ...
  const confirmDelete = async () => {
    try {
      if (customerToDeleteId) {
        ...
        toast.success('Zákazník bol odstránený');
  ...
  if (!user) {
    if (showBookingForm) {
      return <PublicBookingForm onBack={() => setShowBookingForm(false)} />;
    }
